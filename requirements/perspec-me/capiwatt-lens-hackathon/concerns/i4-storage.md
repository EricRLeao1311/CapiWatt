...
- Layout físico (bucket/volume, chaves por `document_version`) — issue #8.
- Store concreto do backend para `document_relations` (DynamoDB por plano vs. relacional local) e como a consulta de grafo por profundidade é servida — registrado como comentário na issue #8.
