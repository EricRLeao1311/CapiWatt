...
**Sugestão de fusão** = estado de ingestão guardado pelo `backend` (F3): par de famílias, score de similaridade, `status: pending | accepted | rejected`, quem e quando decidiu. Gerada ao fim do job de ingestão quando a nova versão não tem chave explícita e passa o limiar contra uma família existente. Confirmada inline na página da família ou pelo atalho no card de resultado — **não** em tela de ingestão/revisão nem em fila de revisão separada (issue-5, [[u4-visualization]], [[u6-acceptance]]) —, por qualquer usuário autenticado no hackathon, com registro de autoria. **Nível de aplicação (2026-09-25):** esta é a regra de curadoria pretendida. O TB1 não tem autenticação nem sessão — `user_id` é uma string de usuário demo fixa (`hackathon/backend/app/models.py:143-148`) —, então o autor registrado no TB1 é esse usuário demo. Quais perfis podem confirmar, quando existirem pessoas distintas, continua aberto em [[u6-acceptance]]. Aceitar chama `reassign_family(document_version, family_id)` no serviço vetorial (refinamento do contrato da issue #2, [[i9-integration]]).
