...
type ExploreState = { kind: "loading" } | { kind: "result"; data: ExploreData } | { kind: "error" };
const tabs = ["Resultados e ranking", "Lacunas da pesquisa", "Análise por tema", "Documentos relacionados"] as const;

export function ExplorePage() {
  const [params] = useSearchParams();
  const initialQuery = params.get("q") || mockExploreData.query;
  ...
  useEffect(() => { runSearch(initialQuery); }, [initialQuery]);
  ...
  return (
    <div className="explore-page">
      <header className="explore-hero">
        ...
        <form className="explore-search" onSubmit={submit}><Search size={21} /><input aria-label="Consulta de precedentes" value={query} onChange={(event) => setQuery(event.target.value)} /><button className="yellow-button" type="submit">Buscar</button></form>
      ...
      <nav className="explore-tabs" aria-label="Visões da pesquisa">{tabs.map((tab) => <button type="button" key={tab} className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tab}</button>)}</nav>
